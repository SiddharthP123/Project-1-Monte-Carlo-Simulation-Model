...
The starting point. Given a fixed list of 10 daily closing prices, this script manually computes both types of daily returns and then measures how much they vary (volatility).
...
- Calculates daily and annualised volatility (sigma).
- Calculates daily and annualised drift (mu).

**Key insight:** Volatility scales with the *square root* of time, while drift scales *linearly*. This is why you multiply daily volatility by √252 to annualise it, but multiply daily drift by 252.
...
Any process that involves randomness evolving. Stock prices are stochastic — you cannot know the next price with certainty, only describe the probability distribution of where it might go.

**b) Wiener Process (Brownian Motion):**
...
### 6. The Merton Jump Diffusion Model:
